...
- [ ] **`city.visaOffice` 必须从城市级字段改成校区级引用。** 调研已证明南京有多个出入境受理点，去哪一个取决于学校和住址，南航把学生带去白下路 173 号，江宁的学生可能走江宁行政服务中心，仙林大学城内另有受理点。一个城市级字段在结构上无法表达这件事，所以它必然对某些校区是错的。改成每个 `Campus` 引用自己那个 slug，城市页列出全部受理点。`docs/sources/nuaa-jiangning.md` 里有来源。
...
- [ ] **先读 `docs/sources/nuaa-jiangning.md`，它已经预填好了。** 那份清单给出九步里前四步的官方 URL 与原文，并且点明后五步（手机卡、银行卡、移动支付、校园卡、医保金额）南航官方页面查不到，只能实地采集。它决定这个 PR 的真实工作量。
- [ ] 派出所已查到，将军路校区国际学生到**江宁开发区派出所，通淮街 99 号**，来源是南航居留许可页。这是四个校区级地点里唯一已有官方来源的一个，直接用，不要重新猜。办公时间与是否需预约仍缺，实地补。
- [ ] 出入境受理点已查到，南航统一带学生去**白下路 173 号**，江宁的学生可能走**江宁行政服务中心，杨家圩路 2 号**。不要沿用现有内容里的江东中路 265 号，那是市政务服务中心公安大厅，是真实受理点但不是南航去的那个。
- [ ] **`CampusFacts` 不要照抄现有六个校区的取值。** 现有六个全部写 `dormGuaranteed: true` 与 `offCampusAllowed: true`，而南航官方页面写校外住宿「原则上不允许」，宿舍也查不到保证有床。留学生规模只有旧数（十三五末年均约 1400），生源国 118 是累计不是在校，若要填这两个字段必须在页面上标明口径与年份，否则是另一种误导。
...
- [ ] `docs/sources/nuaa-jiangning.md` 已有初稿，你的任务是补它而不是重写它。每加一条内容就在清单里加一行 URL 与核验日期，实地采集回来的条目标注采集人与日期。这份文件是下一次复核的起点。
